...
class dict_entry(pydantic.BaseModel): 
   ...
   # # @validator(pre=True)
   # def nest_payloads(cls,v):
   #    payloads = ['paragraph_payload','lemma_payload','root_payload']
   #    for key in payloads:
   #       payload = v.get(key,False)
   #       if payload and not isinstance(payload,dict):
   #          v[key] = {key:payload,'irregularities':[],'masks':[]}
   #    return v        

   # @root_validator(pre=True)
   # # @validator(pre=True)
   # def check_paragraphs_constraint(cls,v):
   #    if not getattr(cls, 'paragraph_payload',False):
   #       print('looking for pragraphs...',v)
   #       assert 'paragraph_payload' in v, 'dict entry cannot be created without specifying the list of Docx_Paragraph_and_Runs objects'
   #    if not getattr(cls, 'lemma',False):
   #       assert 'lemma_payload' in v, 'dict entry cannot be created without specifying the list of lemma_payload objects'
   #    if not getattr(cls, 'root',False):
   #       assert 'root_payload' in v, 'dict entry cannot be created without specifying the list of root_payload objects'
   #    if 'irregularities' not in v:
   #       v['irregularities'] = getattr(cls, 'irregularities',[])
   #    if 'unused_content' not in v:
   #       v['unused_content'] = getattr(cls, 'unused_content',[])
   #    print(v.items())
   #    return v
   ...
   def self_setter(self,attr_name,attr_body,append=False):
      if append:
         is_set = self.dict(exclude_unset=True,exclude_defaults=True).get(attr_name,None)
         print(is_set)
         if is_set is not None:
            is_set.append(attr_body)
      else:
         setattr(self,attr_name,attr_body)
...
def parse_dataframe_groups(workinglemmas, parsed_object_lookup, eDP:pd.DataFrame):
   ...
   for e in workinglemmas.items():
      ...
      values['paragraph_payload']= [int(lemma)]+paras
